...
function Comparison({title,description,before,after}:{title:string;description:string;before:ReactNode;after:ReactNode}) {
 ...
 return <figure className="comparison-card">
  ...
  <p className="comparison-project-detail">{description}</p>
 </figure>;
}
export function BeforeAfter({chimneyBefore,chimneyAfter,patioBefore,patioAfter}:{chimneyBefore:ReactNode;chimneyAfter:ReactNode;patioBefore:ReactNode;patioAfter:ReactNode}) {
 return <section className="section wrap comparison-section" id="results" aria-labelledby="results-title">
  ...
  <div className="comparison-grid"><Comparison title="Stone chimney" description="Exterior stone cleaning: compare the visible surface buildup before and after Apex’s work." before={chimneyBefore} after={chimneyAfter}/><Comparison title="Fire-pit patio" description="Patio cleaning: a view of the stone around the fire pit before and after cleaning." before={patioBefore} after={patioAfter}/></div>
